...
from utils.inception_score import _init_inception, create_inception_graph
...
def main():
    ...
    if args.optimizer == 'Adam':
        ...
        dis_optimizer = sLead_Adam(filter(lambda p: p.requires_grad, dis_net.parameters()),
                                   args.d_lr, (args.beta1, args.beta2), alpha_vjp=args.alpha_vjp_d)
        gen_optimizer = sLead_Adam(filter(lambda p: p.requires_grad, gen_net.parameters()),
                                   args.g_lr, (args.beta1, args.beta2), alpha_vjp=args.alpha_vjp_g)
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        ...
        if epoch and epoch % args.val_freq == 0 or epoch == int(args.max_epoch) - 1:
            ...
            inception_score, fid_score = validate(args, fixed_z, gen_net, writer_dict, train_loader, epoch)
